 ...
 * in Mazes/Woods1.txt
 */
public class DSTF_TraceReset_maze implements Environment {
	/**
	 ...
	 */
	public DSTF_TraceReset_maze(String mazeFile) throws IOException {
	 ...
	 *             On standard IO problems
	 */
	public DSTF_TraceReset_maze(File f) throws IOException {
...
//		}
		...
		openLocations.add(new Point(1, 2));
		openLocations.add(new Point(5, 2));
		openLocations.add(new Point(1, 3));
		openLocations.add(new Point(5, 3));
		
		openLocations.add(new Point(1, 4));
		openLocations.add(new Point(2, 4));
		openLocations.add(new Point(3, 4));
		openLocations.add(new Point(4, 4));
		openLocations.add(new Point(5, 4));
		
		
		finalStates.add(new Point(2, 2));
		finalStates.add(new Point(4, 3));
	...
	public static void main(String[] args) throws IOException {
		...
		try {
			...
			for (int z = 0; z < totalCalcTimes; z++) {
				...
				DSTF_TraceReset_maze maze = new DSTF_TraceReset_maze("data/DSThalf3bit.txt");
				maze.resetPosition();
...
//				params.pHash = 0.;
				...
				int i = 1;
				while (finalStateCount < 701) {
					nxcs.runIteration(finalStateCount, maze.getState());
					i++;

					if (finalStateCount % 700 == 0 && !logged) {
...
//						}
					...
					if (maze.isEndOfProblem(maze.getState())) {
						maze.resetPosition();
						if(finalStateCount==499||finalStateCount==500||finalStateCount==501||finalStateCount==502||finalStateCount==503||finalStateCount==504){
							...
							for (Point p : maze.openLocations) {
								...
								Collections.sort(C, (a, b) -> (int) ((a.action - b.action) * 1024));
								for (Classifier c : C) {
									System.out.println("clas: " + c);
								}

							}
							System.out.println("finalStateCount:"+finalStateCount+":over");
						
						
						}
//						if(i==3||i==1||i==2||i==4||i==5||i==6||i==7||i==8||i==9){
//							System.out.println("i:"+i+":print classifiers for each openlocations");
//							for (Point p : maze.openLocations) {
//								System.out.println("x:" + p.x + " y:" + p.y);
//								List<Classifier> C = nxcs.getMatchSet(maze.getStringForState(p.x, p.y));
//								Collections.sort(C, (a, b) -> (int) ((a.action - b.action) * 1024));
...
//					maze.y=1;
	...
	public HashMap<Point, Result> GetResult(DSTF_TraceReset_maze maze, NXCS nxcs) {
		int finalStateCount2 = 0;
		int timestamp = 0;
	...
	public HashMap<Point, Result> trace(DSTF_TraceReset_maze maze, NXCS nxcs) {
		int finalStateCount2 = 0;
		int timestamp = 0;
		HashMap<Point, Result> resultMap = new HashMap<Point, Result>();
