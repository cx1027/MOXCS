 ...
 * in Mazes/Woods1.txt
 */
public class Donkey implements Environment {
	/**
	...
	private int appearF1 = 0;
	private int appearF3 = 0;
	private int eat = 0;
	 ...
	 */
	public Donkey(String mazeFile) throws IOException {
	 ...
	 *             On standard IO problems
	 */
	public Donkey(File f) throws IOException {
		...
		actions.add(new Point(0, 0));// Not move
	...
	public Qvector getReward(String state, int action) {
		...
		//reward=[eat,stole,walk]
		Qvector reward = new Qvector(0,0,0);
		...
		//eat	
		if((x==1 && y==1)&&(flagF1==true)){
			eat=0;
			appearF1=0;
			flagF1=false;
		} else if((x==3 && y==3)&&(flagF3==true)){
			eat=0;
			appearF3=0;
			flagF3=false;
		}else{
			...
			if(appearF1>9){flagF1=true;}
			if(appearF3>9){flagF3=true;}
		}
		
		//stole
		if((x==1 && y==3)||(x==3 && y==1)){
			if(random<90){//90% get stolen
				reward.set(1, -1d);
			}
		}else if(x==2 && y==2){
			reward.set(1, 0d);
	...
	public static void main(String[] args) throws IOException {
		...
		try {
			...
			for (int z = 0; z < totalCalcTimes; z++) {
				...
				Donkey maze = new Donkey("data/Donkey.txt");
				maze.resetPosition();
				...
				params.intQ = new ArrayList<Qvector>();
				params.intQ.add(new Qvector(0,0,0));
				params.intV = new ArrayList<Qvector>();
				params.intV.add(new Qvector(0,0,0));
				params.intR = new Qvector(0,0,0);
				...
				int i = 0;
				while (finalStateCount < 101) {
					nxcs.runIteration(i, maze.getState());
					i++;

//					if (finalStateCount % 100 == 0 && !logged) {
...
//					maze.y=1;
	...
	public HashMap<Point, Result> GetResult(Donkey maze, NXCS nxcs) {
		int finalStateCount2 = 0;
		int timestamp = 0;
	...
	public HashMap<Point, Result> trace(Donkey maze, NXCS nxcs) {
		int finalStateCount2 = 0;
		int timestamp = 0;
		HashMap<Point, Result> resultMap = new HashMap<Point, Result>();
