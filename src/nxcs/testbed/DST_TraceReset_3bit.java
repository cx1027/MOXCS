 ...
 * in Mazes/Woods1.txt
 */
public class DST_TraceReset_3bit implements Environment {
	/**
	 ...
	 */
	public DST_TraceReset_3bit(String mazeFile) throws IOException {
	 ...
	 *             On standard IO problems
	 */
	public DST_TraceReset_3bit(File f) throws IOException {
	...
	public static void main(String[] args) throws IOException {
		...
		try {
			...
			for (int z = 0; z < totalCalcTimes; z++) {
				...
				DST_TraceReset_3bit maze = new DST_TraceReset_3bit("data/DST3bit.txt");
				maze.resetPosition();
...
//				params.pHash = 0.5;
				...
				int i = 0;
				while (finalStateCount < 151) {
					nxcs.runIteration(i, maze.getState());
					i++;

					if (finalStateCount % 150 == 0 && !logged) {
...
//					maze.y=1;
	...
	public HashMap<Point, Result> GetResult(DST_TraceReset_3bit maze, NXCS nxcs) {
		int finalStateCount2 = 0;
		int timestamp = 0;
	...
	public HashMap<Point, Result> trace(DST_TraceReset_3bit maze, NXCS nxcs) {
		int finalStateCount2 = 0;
		int timestamp = 0;
		HashMap<Point, Result> resultMap = new HashMap<Point, Result>();
