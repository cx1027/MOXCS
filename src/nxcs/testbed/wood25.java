 ...
 * in Mazes/Woods1.txt
 */
public class wood25 implements Environment {
	/**
	 ...
	 */
	public wood25(String mazeFile) throws IOException {
	 ...
	 *             On standard IO problems
	 */
	public wood25(File f) throws IOException {
	...
	public static void main(String[] args) throws IOException {
		...
		try {
			...
			for (int z = 0; z < totalCalcTimes; z++) {
				...
				wood25 maze = new wood25("data/woods2.txt");
				maze.resetPosition();
				...
				int i = 1;
				while (finalStateCount < 1002) {
					nxcs.runIteration(finalStateCount, maze.getState());
					i++;

					if (finalStateCount % 1001 == 0 && !logged) {
					...
					if (maze.isEndOfProblem(maze.getState())) {
						maze.resetPosition();
						if(finalStateCount==300||finalStateCount==1000||finalStateCount==903){
...
//				}
	...
	public HashMap<Point, Result> GetResult(wood25 maze, NXCS nxcs) {
		int finalStateCount2 = 0;
		int timestamp = 0;
	...
	public HashMap<Point, Result> trace(wood25 maze, NXCS nxcs) {
		int finalStateCount2 = 0;
		int timestamp = 0;
		HashMap<Point, Result> resultMap = new HashMap<Point, Result>();
