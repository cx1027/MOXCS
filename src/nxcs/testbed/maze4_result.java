...
public class maze4_result implements Environment {
	...
	public boolean isEndOfProblem(String state) {
		for (Point finalState : finalStates) {
			if (getStringForState(finalState.x, finalState.y).equals(state)) {
				// System.out.println(String.format("finalx,finaly:%d %d",
	...
	public static void main(String[] args) throws IOException {
		...
		try {
			...
			for (int z = 0; z < totalCalcTimes; z++) {
				...
				//TODO:finalStateUpperBound / 20) / 10 * 10 should be 20
				StatsLogger logger = new StatsLogger((int) (finalStateUpperBound / 20) / 10 * 10, 0);
				...
				while (finalStateCount < finalStateUpperBound) {
					...
					if (finalStateCount % resultInterval == 0 && !logged) {
						double hyperSum = 0;
						System.out.println("calculate Pareto sum at every 30 times");
				...
				// TODO:Plot the picture of the whole result!!!!!!!!!!!!!!
				logger.logRun(stats);
