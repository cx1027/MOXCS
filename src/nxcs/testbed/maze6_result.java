				...
				for (String distCalcMethod : discCalcMethods) {
					...
					for (int trailIndex = 0; trailIndex < totalTrailTimes; trailIndex++) {
						...
						try {
							...
							// String.format(
							// "log/maze6/csv/%s/%s/%s - %s - Trial %d -
							// <TRIAL_NUM>-HyperVolumn.csv",
							// "MOXCS", "MAZE6", actionSelectionMethod,
							// distCalcMethod, trailIndex),
							// String.format(
							// "log/maze6/datadump/%s/%s - %s - Trail
							// %d-<TIMESTEP_NUM> - hypervolumn.log",
